...
\begin{center}
\begin{abstract}
  This document documents the mathematical and statistical background of the research and testing I conducted for my strategy development. Explanation of the formulas I used and the different versions of my algorithms.
\end{abstract}
\end{center}
...
\subsection{Orderbook Delta definition}
...
A outlier is defined when a the Orderbook $\Delta_t$ is outside of the normal range. I define the normal rage as the mean $\mu(\Delta)$ plus or minus 2 times the standard deviation $\sigma(\Delta)$. For mean and standard deviation I use a rolling window of 1440 $\Delta$ values. On a timeseries dataset with a interval of 1 minute that is equal to one day.

Equation for outlier definition:
...
\begin{itemize}
  \item I expect realized volatility to increase after an outlier is detected.
...
Now I defined a a bullish outlier as bullish if he has a certain z-score value and a bearish outlier as a bearish if he has a certain z-score value.
...
The first Idea I had was to measure the volatility of the price after an outlier is detected. Volatility is a measure of how much price changes in either direction over a certain period of time. 
...
\section{Optimising for best Z-Score threshold for outliers}

As stated inside of \eqref{eq: outlier_detection} we use a Z-Score threshold of 2 to detect outliers. I now want to 
see if by any chance there is a value where ther realized volatility is higher than if we use 2 as a threshold.
...
First I run an optimization for the thresholds of the Z-Score to find the best threshold value for the outliers on a 45 days dataset.
After that I compare the result with a 107880 minutes dataset. Where I also run an optimization for the thresholds of the Z-Score to find the best threshold value for the outliers.
...
\begin{itemize}
  ...
  \item Build a systematich version of your discretionary trading.
\end{itemize}
...
I had a pretty hard time going from developing a logic for the outliers of the Delta. Finding that volatility increases after outliers was a good first find which didn't take long and the process was pretty straight forward. But finding some definition condtion is meet directional bias was very difficult. By directional bias I mean a price direction which is not random after some kind of condition. It was pretty clear from the beginning on that the outliers by there selfe won't offer any kind of directional bias
...
We basically got through the time series dataset and look back $n$ $P_t$ values. The highest and lowest points inside of that specific lookback period are the swing points. After that we detetermine if price is making higher highs or lower lows. We check this by storeing the last swing points and wait till we are either making a higher high or a lower low.
...
Now since my strategy allows multiple positions open at the same time the infrastructure turns out to be a bit more complex. If we open two positions on the $BTC/USD$ pair with a size of 1000\$ we end up having one positions with a size of 2000\$. 
...
I decided to handle the issues of multiple positions with different exit times by using an asynchronous approach. Every positions is handled in a separate python thread. The thread just sells the exact amount of one single position which is $10\%$ of the portfolio. The maximum of open positions is set to 20.


The algorithm uses a class \footnote{A class in Python is a blueprint for creating objects. It defines a set of attributes and methods that an object of that class will have.} called AsyncPositionManager. The position manager initializes is initialized as part of the signal loop which is responsible to check every minute if an entry signal is detected. If an entry signal is detected the position manager will open a position which is handled inside of an asynchronous position task that runs independently.
...
\newpage
\section{Code implementation for data creation}
...
\subsection{Handling Time Series Alignment in High-Frequency Data Collection}
...
\begin{itemize}
    \item API request latency (~100ms) causing timestamp misalignment
    \item Need for regular intervals (xx:00:00) versus actual recording times (xx:00:15)
    \item Risk of missing data points affecting backtest validity
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Continuous Data Collection Layer}
    \begin{itemize}
        \item Implements parallel API requests using ThreadPoolExecutor
        \item Maintains a bounded queue (maxsize=1000) to buffer data points
        \item Collects data at 2-second intervals to ensure sufficient sampling density
    \end{itemize}

    \item \textbf{Time Series Alignment Layer}
    \begin{itemize}
        \item Aligns timestamps to 10-second boundaries:
        \begin{verbatim}
        ...
        \end{verbatim}
        \item Tracks collection latency for each data point:
        \begin{verbatim}
        collection_latency = (current_timestamp - 
                            last_data.timestamp).total_seconds()
        \end{verbatim}
        \item Flags interpolated data points when latency exceeds threshold:
        \begin{verbatim}
...
\begin{itemize}
    \item Records collection latency in the database for each data point
    \item Marks data points as interpolated when timing constraints are violated
    \item Provides transparency for backtest validation by tracking data quality metrics
\end{itemize}
...
A lot of my ideas come from thiy guys Tweets, to research into this. He is a discretionary trade what means he trades based on his own decisions and has a mental framework to trade. I learned what an orderbook delta is from him.
...
Altho this book is for a discretionary trading approach it taught me a lot about the Efficient Market Hypothesis and trading psychology.
